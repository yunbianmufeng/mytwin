    ...
    this.initPromise = new Promise((resolve, reject) => {
      try {
        ...
        request.onupgradeneeded = (event) => {
          ...
          // 创建所需的所有存储对象
          const stores = ['processes', 'products', 'productConfigs']
        ...
        request.onsuccess = () => {
          ...
          // 验证所有存储对象是否存在
          const stores = ['processes', 'products', 'productConfigs']
          const missingStores = stores.filter(store => !this.db.objectStoreNames.contains(store))
          if (missingStores.length > 0) {
            const error = new Error(`Missing stores: ${missingStores.join(', ')}`)
            this.initPromise = null
            reject(error)
            return
          }
