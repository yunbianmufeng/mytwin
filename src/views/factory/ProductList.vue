...
const simulateProduction = (productLine) => {
  console.log('跳转到模拟仿真页面:', productLine)
  
  // 携带产线数据跳转到模拟仿真页面
  const productionLineDataStr = encodeURIComponent(JSON.stringify(productLine.productionLineData))
  
  router.push({
    path: '/factory/simulation',
    query: {
      productionLineData: productionLineDataStr,
      productLineName: productLine.productionLine.name,
      productLineId: productLine.productionLine.id
    }
  })
}
...
// 获取可选的工步列表
const getAvailableSteps = (productionLineId) => {
  const productionLine = productionLines.value.find(line => line.id == productionLineId)
  if (!productionLine || !productionLine.layers) return []
  ...
  productionLine.layers.forEach(layer => {
    if (layer.steps) {
      layer.steps.forEach(step => {
        steps.push({
          value: step.processName || '未知步骤',
          label: step.processName || '未知步骤'
        })
      })
    }
  })
...
// 开始编辑生产状态
const startEditStatus = (productLine) => {
  const currentStatus = getCurrentProductionStatus(productLine.originalConfig)
  editingStatus.value[productLine.id] = true
  editFormData.value[productLine.id] = {
    currentNumber: currentStatus.currentNumber,
    currentStep: currentStatus.currentStep,
    startTime: productLine.originalConfig.startDate,
    estimatedEndTime: currentStatus.estimatedEndTime === '已完成' || currentStatus.estimatedEndTime === '未知' 
      ? '' : currentStatus.estimatedEndTime,
    remarks: productLine.originalConfig.remarks || ''
  }
}

// 取消编辑
const cancelEditStatus = (productLineId) => {
  editingStatus.value[productLineId] = false
  delete editFormData.value[productLineId]
}

// 保存生产状态编辑
const saveProductionStatus = async (productLine) => {
  try {
    const editData = editFormData.value[productLine.id]
    if (!editData) return
    
    // 更新产品配置数据
    const updatedConfig = {
      ...productLine.originalConfig,
      currentNumber: parseInt(editData.currentNumber) || productLine.originalConfig.startNumber,
      startDate: editData.startTime,
      remarks: editData.remarks
    }
    
    // 调用store更新数据
    await store.dispatch('product/updateProductConfig', updatedConfig)
    
    ElMessage.success('生产状态更新成功')
    
    // 退出编辑模式
    editingStatus.value[productLine.id] = false
    delete editFormData.value[productLine.id]
    
    // 重新加载数据
    await initData()
    
  } catch (error) {
    console.error('保存生产状态失败:', error)
            ...
            <template #default="props">
              <el-tabs>
                ...
                <el-tab-pane label="当前生产状态">
                  <!-- 编辑模式 -->
                  <div v-if="editingStatus[props.row.id]" class="edit-status-form">
                    <el-form 
                      :model="editFormData[props.row.id]" 
                      label-width="120px"
                      style="max-width: 800px"
                    >
                      <el-row :gutter="20">
                        <el-col :span="12">
                          <el-form-item label="当前晶圆编号">
                            <el-input-number 
                              v-model="editFormData[props.row.id].currentNumber"
                              :min="parseInt(props.row.originalConfig.startNumber)"
                              :max="parseInt(props.row.originalConfig.endNumber)"
                              style="width: 100%"
                            />
                          </el-form-item>
                        </el-col>
                        <el-col :span="12">
                          <el-form-item label="当前工步">
                            <el-select 
                              v-model="editFormData[props.row.id].currentStep"
                              style="width: 100%"
                              placeholder="请选择当前工步"
                            >
                              <el-option
                                v-for="step in getAvailableSteps(props.row.productionLine.id)"
                                :key="step.value"
                                :label="step.label"
                                :value="step.value"
                              />
                      ...
                      <el-row :gutter="20">
                        <el-col :span="12">
                          <el-form-item label="开始时间">
                            <el-date-picker
                              v-model="editFormData[props.row.id].startTime"
                              type="datetime"
                              placeholder="选择开始时间"
                        ...
                        <el-col :span="12">
                          <el-form-item label="预计结束时间">
                            <el-date-picker
                              v-model="editFormData[props.row.id].estimatedEndTime"
                              type="datetime"
                      ...
                      <el-form-item label="备注">
                        <el-input 
                          v-model="editFormData[props.row.id].remarks"
                          type="textarea"
                          :rows="3"
                          placeholder="请输入备注信息"
                        />
                      ...
                      <el-form-item>
                        <el-button type="primary" @click="saveProductionStatus(props.row)">
                          保存
                        </el-button>
                        <el-button @click="cancelEditStatus(props.row.id)">
                          取消
                        </el-button>
                      </el-form-item>
                    </el-form>
                  ...
                  <div v-else>
                    <div class="status-header" style="margin-bottom: 15px;">
                      <el-button type="primary" size="small" @click="startEditStatus(props.row)">
                        编辑生产状态
                      </el-button>
                    </div>
                    ...
                    <el-descriptions border :column="2">
                      ...
                      <el-descriptions-item label="备注" :span="2">
                        {{ props.row.originalConfig.remarks || '无' }}
                      </el-descriptions-item>
                    </el-descriptions>
                  </div>
                </el-tab-pane>
