...
const defaultStep = {
  name: '光刻',
  progress: 60,
  startTime: '2025-08-12 09:30:00',
  estimatedEndTime: '2025-08-12 11:30:00'
}

const defaultWafer = {
  completionTime: '2025-08-01 15:30:00',
  materialConsumption: [
    { name: '光刻胶', amount: 0.05, unit: 'L' },
    { name: '超纯水', amount: 1, unit: 'L' },
    { name: '显影液', amount: 0.03, unit: 'L' }
  ],
  steps: [
    { name: '清洗', startTime: '2025-08-01 10:00:00', endTime: '2025-08-01 11:00:00' },
    { name: '光刻', startTime: '2025-08-01 11:30:00', endTime: '2025-08-01 13:30:00' },
    { name: '显影', startTime: '2025-08-01 14:00:00', endTime: '2025-08-01 15:30:00' }
  ]
}
...
const processConfig = (config) => {
  ...
  const materialConsumption = productionLine ? getMaterialConsumptionFromProductionLine(productionLine) : []
  ...
  return {
    ...
    productionLine: {
      ...
      name: config.productionLineName
    },
    ...
    currentTime: 48,
    expectedEndTime: formatDate(config.expectedEndTime) || '2025-08-11',
    deadline: formatDate(config.deadline) || '2025-08-15',
    currentStep: defaultStep,
    completedWafers: [completedWafer]
  }
}

// 从工艺线获取物料消耗信息
const getMaterialConsumptionFromProductionLine = (productionLine) => {
  // 根据工艺线的步骤计算物料消耗
  const materials = []
  
  if (productionLine && productionLine.steps) {
    productionLine.steps.forEach(step => {
      // 根据不同的工艺步骤添加对应的物料消耗
      switch (step.processName) {
        case '清洗':
          materials.push(
            { name: '超纯水', amount: 10, unit: 'L' },
            { name: '清洗剂', amount: 0.5, unit: 'L' }
          )
          break
        case '光刻':
          materials.push(
        ...
        case '氧化':
          materials.push(
            { name: '氧气', amount: 50, unit: 'L' },
            { name: '硅烷', amount: 0.1, unit: 'L' }
          )
          break
      }
    })
...
const getCompletedWaferDetails = (config) => {
  ...
  for (let i = 0; i < completedQuantity; i++) {
    ...
    completionDate.setHours(18, 0, 0, 0) // 假设每天18:00完成
  ...
  <div class="product-list">
    ...
    <el-card style="margin-top: 5px;">
      ...
      <el-table :data="[productLine]" style="width: 100%" border>
        ...
        <el-table-column prop="currentTime" label="当前用时(h)" width="120"></el-table-column>
        <el-table-column prop="expectedEndTime" label="预计结束" width="120"></el-table-column>
        <el-table-column prop="deadline" label="最后期限" width="120"></el-table-column>
