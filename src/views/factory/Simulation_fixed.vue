  ...
  <div class="simulation-page">
    ...
    <el-card style="margin-top: 20px;" v-if="showDebugInfo">
      ...
      <el-tabs>
        ...
        <el-tab-pane label="访问信息">
          <pre>当前访问地址: {{ webglUrl }}
协议: {{ window.location.protocol }}
主机: {{ window.location.host }}
路径: {{ window.location.pathname }}</pre>
        </el-tab-pane>
      </el-tabs>
    </el-card>
  </div>
...
const showDebugInfo = ref(true) // 默认显示调试信息以便查看问题
const webglIframe = ref(null)

// WebGL相关配置 - 动态生成URL确保局域网访问一致
const webglUrl = computed(() => {
  const protocol = window.location.protocol
  const host = window.location.host
  return `${protocol}//${host}/webgl/index.html`
})
...
const webglHeight = computed(() => {
  ...
  return `calc(100vw * ${aspectRatio} - 40px)` // 减去页面padding
})
...
const onWebGLLoaded = async () => {
  console.log('WebGL加载完成，当前URL:', webglUrl.value)
...
onMounted(() => {
  ...
  console.log('Simulation页面挂载完成，当前WebGL URL:', webglUrl.value)
})
